...
<body>
            ...
            <form method="post" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>">
              <div class="row">
                <div class="col-md-5 form-group <?php echo (!empty($oldpassword_err)) ? "has-error" : ''; ?>">
                  <label>舊有密碼</label>
                  <input class="form-control" type="password" name="oldpassword" id="oldpassword_id">
              ...
              <div class="row">
                <div class="col-md-5 form-group <?php echo (!empty($newpassword_err)) ? "has-error" : ''; ?>">
                  <label>新密碼</label>
                  <input class="form-control" type="password" name="newpassword" id ='password_id'>
              ...
              <div class="row">
                <div class="col-md-5 form-group <?php echo (!empty($oldpassword_confirmation_err)) ? "has-error" : ''; ?>">
                  <label>新密碼確認</label>
                  <input class="form-control" type="password" name="newpassword_confirmation" id='password_confirmation_id'>
              ...
              <div class="row">
                <div class="col-md-10 form-group">
                  ...
                  <button type="submit" id ='subButton' class="btn submitButton" disabled="true">確認</button>
                </div>
              </div>
            </form>
          </div>
  ...
  <script>
    var password_id_vali = false;
    var password_confirmation_id_vali = false;
    function valithru(){
      if(password_id_vali && password_confirmation_id_vali){
    ...
    $('#password_id').on('keyup', function(){
      if($(this).val().length < 6){
        ...
        $('.emsg_password').html(' ');
        password_id_vali = true;
        valithru();
      }
    });
    $('#password_confirmation_id').on('keyup', function(){
      if($(this).val() == $('#password_id').val()){
        $('.emsg_password_confirmation').html(' ');
        password_confirmation_id_vali = true;
        valithru();
      }
      else{
